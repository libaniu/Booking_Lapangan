...
// Ambil nama dari tabel users berdasarkan username yang sedang login
$session_username = $_SESSION["username"];
$sqlUser = "SELECT name FROM users WHERE username = '$session_username'";
$resultUser = $conn->query($sqlUser);
$nama_penyewa = $session_username; // Default fallback ke username jika name belum diset
if ($resultUser && $resultUser->num_rows > 0) {
    $rowUser = $resultUser->fetch_assoc();
    if (!empty($rowUser['name'])) {
        $nama_penyewa = $rowUser['name'];
    }
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO formsewa (order_id, nama, id_lapangan, tanggal, jam_mulai, jam_selesai, total_bayar, status_booking) 
    VALUES ('$order_id', '$nama', '$id_lapangan', '$tanggal', '$jam_mulai', '$jam_selesai', '$total_bayar', '$status_booking')";
                                    ...
                                    <div class="row">
                                        <div class="col-12">
                                            <div class="form-group">
                                                ...
                                                <input type="text" id="nama" class="form-control" name="nama" value="<?php echo htmlspecialchars($nama_penyewa); ?>" readonly required>
                                            </div>
                                        </div>
                                        <div class="col-12">
                                            <div class="form-group">
